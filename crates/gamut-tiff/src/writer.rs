...
/// page is `(directory, strips)`; `StripOffsets`/`StripByteCounts` are filled in here.
#[must_use]
pub fn write_multipage(order: ByteOrder, pages: &[(Ifd, Vec<Vec<u8>>)]) -> Vec<u8> {
    use crate::ifd::Value;
    use crate::tags;

    // Give each directory its byte counts and a correctly-sized StripOffsets placeholder so the
    // whole directory block's layout (and thus the strip-data base) is final.
    ...
    let mut out = write(&TiffFile { order, ifds });
    out.resize(base, 0);
