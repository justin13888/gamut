...
use gamut_avif::{AvifContainer, UnknownBoxLocation};

/// A hand-authored file whose `meta` carries an unconsumed `uuid` child, and whose `iprp` carries
/// an unconsumed `free` child alongside the real `ipco`/`ipma`. `read` must still parse it, and
/// the two unknown boxes must surface with their exact bodies while the known children stay
/// unreported.
#[test]
fn unknown_meta_and_iprp_children_surface_verbatim() {
    ...
    let m = meta(&[
        hdlr(),
        pitm_v0(1),
        iinf_v0(&[infe_v2(1, b"av01", false)]),
        iprp,
        uuid,
    ]);
