...
lossy AV1 intra encoding at 4:4:4, 4:2:2, 4:2:0 or monochrome, 8/10/12-bit, from
`Rgb8`/`Rgba8`/`Gray8`/`Rgb16`/`Rgba16` — identity matrix at 4:4:4 for lossless, **BT.709 YCbCr at
4:2:0 by default** for lossy, with BT.601/BT.2020-NCL, studio range and 4:4:4 / 4:2:2 selectable —
wrapped as a conformant MIAF/AVIF `av01` item (plus a monochrome alpha auxiliary when the input
carries one) — brands `avif`/`mif1`/`miaf` plus whichever profile brand *every* image item in the
file earns (`MA1B` when all are AV1 Main at level ≤ 5.1, `MA1A` when all are High at ≤ 6.0, neither
otherwise — which is the case for 4:2:2, for 12-bit, and for a High-profile colour item paired with
its Main-profile alpha auxiliary), the AVIF §9.1.1 minimum box set, cross-box
...
**Deferred (planned, additive).** Every ☐ row below: depth auxiliary items; the HDR
...
| seq_profile=0 (Main) / =2 (Professional, 12-bit/4:2:2) | Annex A §10.2 | ✅ (0 = 8/10-bit 4:2:0 **and monochrome**; 2 = 4:2:2 at any depth **and** every 12-bit layout) | M2 |
...
| high-bit-depth **input**: `Rgb16`/`Rgba16` narrowed to the coded depth (`AvifEncoder::with_bit_depth`, default 12-bit, truncating), at any chroma format | AVIF §2.2 | ✅ (#399) | M2 |
| color_config: `mono_chrome` bit + its inferred-subsampling branch, profile-inferred subsampling, `chroma_sample_position` (`CSP_UNKNOWN`, coded for 4:2:0) | §5.5.2 | ✅ (`gamut_av1::headers`) | M2 |
| color_config: high_bitdepth/twelve_bit, and profile 2's coded `subsampling_x`/`subsampling_y` at 12 bits | §5.5.2 | ✅ (`gamut_av1::headers`) | M2 |
...
| Monochrome on `Av1EncodeRequest` (so a backend can encode, or decline, an alpha auxiliary or a `Gray8` primary — today those go straight to the `gamut-av1` tail, since the seam's contract is a three-plane colour item) | — | ☐ | M3 |
| Chroma on `Av1EncodeRequest` (`chroma()`), validated against the returned stream's `seq_profile` | AV1-ISOBMFF §2.3.4 | ✅ | M2 |
| Bit depth on `Av1EncodeRequest` (`bit_depth()`) + the defaulted `Av1StillEncoder::encode_still16` (whose default **declines**, so an 8-bit backend falls through rather than being handed 10/12-bit planes) | — | ✅ (#399) | M2 |
