...
//! [`Planar8`] / [`Yuv420`] / [`YcbcrMatrix`], the [`clip_pixel`] / [`rgb_to_ycbcr`] helpers, and
//! the colour-science
...
//!   which is outside the AV1 profile set entirely and exists for the 16-bit still-image pipelines
//!   that share these types); the subsampled formats ([`ChromaSubsampling::Cs422`] / `Cs420` /
//!   `Cs400`); [`MatrixCoefficients::YCgCo`], the one modeled matrix with no de-matrixing (it is a
//!   lifting transform, not a `Kr`/`Kb` matrix); and the HLG / BT.709 transfer curves
