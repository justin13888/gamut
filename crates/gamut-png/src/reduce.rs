...
fn build_indexed(
    ...
    let trns = if palette.iter().any(|c| c[3] != 255) {
        ...
        //
        // No length guard: this arm runs only when `needs_trns` held, i.e. some entry's alpha is
        // not 255, so the `last() == 255` test always halts the loop before the vector empties.
        // The `alphas.len() > 1` that used to be here therefore decided nothing, and `>` vs `>=`
        // was an equivalent mutant no test could kill (#110) -- removed rather than excluded.
