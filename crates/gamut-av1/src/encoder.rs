...
/// Validates the source layout, depth-independent, shared by every entry point; returns whether the
/// frame is monochrome.
///
/// Two rules, both §6.4.2/§5.5.2 conformance rather than encoder limits, plus one that *is* an
/// encoder limit:
///
/// - `MC_IDENTITY` requires `subsampling_x == subsampling_y == 0`. The identity matrix carries
///   R'G'B' directly and the three colour planes cannot be sampled at different rates; §5.5.2 says
///   the same structurally, since its sRGB shortcut infers 4:4:4 whatever the profile and its
///   monochrome branch infers 1/1. `Av1Colour::default()` is identity, so a caller reaching here
///   with subsampled or monochrome planes and the default colour would otherwise emit a stream
///   libaom and dav1d are entitled to reject; `Av1Colour::monochrome()` is the fix for the
///   monochrome case.
/// - The **lossless** path is 4:4:4 or monochrome only. §5.11.45 makes `is_cfl_allowed` under
///   `Lossless` the test `Subsampled_Size[MiSize][subX][subY] == BLOCK_4X4`, which at 4:2:0 is true
///   for 4x8, 8x4 and 8x8 as well as 4x4 — while the tile encoder tests `bw == 4`. Coding a
///   subsampled lossless block would emit `uv_mode` against the wrong CDF and desynchronise the
///   decoder's symbol reader, so the combination is refused rather than silently mis-coded.
///
/// The sample depth is orthogonal to all of this and is checked in [`still_config`] instead.
fn check_layout(
    width: u32,
    height: u32,
...
/// The sequence-header field values for one still: the §6.4.1 profile the chroma format **and** the
/// sample depth force between them, the depth flags, the subsampling pair, and the CICP triple.
///
/// The two axes are not independent, which is why one function derives the profile from both.
/// §6.4.1 gives Main (0) 4:2:0 and monochrome, High (1) 4:4:4, and Professional (2) 4:2:2 — but
/// only Professional reaches 12 bits at all, so **every** 12-bit stream is profile 2 whatever its
/// layout, while 8- and 10-bit follow the layout alone.
fn still_config(
    width: u32,
    height: u32,
    monochrome: bool,
    ...
    // The 8/10-bit profile of this layout. There is no profile that codes 4:4:4 *and* 4:2:0, which
    // is why a 4:4:4 still cannot be read by a Main-profile-only hardware decoder.
    let layout_profile = match subsampling {
        ...
        // still shares Main with 4:2:0.
        ChromaSubsampling::Cs420 | ChromaSubsampling::Cs400 => 0,
        // `ChromaSubsampling` is `#[non_exhaustive]` and models layouts beyond AV1's (4:1:1, say).
        // AV1 has no profile for one, so it is refused rather than mapped onto a nearby profile.
        _ => {
            return Err(Error::unsupported(
                env!("CARGO_PKG_NAME"),
    ...
    // §6.4.1: Main and High are 8/10-bit only, so 12 bits of any layout — 4:4:4, 4:2:2, 4:2:0 or
    // monochrome — is Professional. This is the one place the depth axis overrides the format axis.
    let seq_profile = if bit_depth == BitDepth::Twelve {
        2
    } else {
        layout_profile
    };
    ...
    let config = Av1StillConfig {
        ...
        monochrome,
        // Inferred, not coded, except under profile 2 at 12 bits (§5.5.2). Monochrome is fixed at
        // 1/1, which is what `ChromaSubsampling::Cs400.subsampling()` returns.
...
/// Encodes **10- or 12-bit** planes — 4:4:4 identity (Y=G, U=B, V=R), subsampled `Y/Cb/Cr`, or a
/// monochrome luma plane — as an AV1 intra keyframe at quantizer `qindex` (`base_q_idx`; `0`
/// selects the lossless path).
///
/// Depth and chroma format both come from the buffer and both feed the profile (§6.4.1): 10-bit
/// keeps the profile its layout implies and sets `high_bitdepth`, while 12-bit of any layout is
/// profile 2 with `twelve_bit`. The 8-bit sibling is [`encode_still_intra_with`]; a [`Planar16`]
/// that carries [`BitDepth::Eight`] is accepted here too and produces exactly the stream that entry
/// point would.
///
...
pub fn encode_still_intra16_with(
    ...
    let monochrome = check_layout(width, height, planes.subsampling(), qindex, colour)?;
