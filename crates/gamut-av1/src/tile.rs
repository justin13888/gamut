...
impl<'a> FrameEncoder<'a> {
    ...
    fn apply_cfl(&self, pred: &mut [i32], sx: usize, sy: usize, alpha: i32, tx_size: TxSize) {
        ...
        for (p, &lv) in pred.iter_mut().zip(&l) {
            // `Clip1` at the frame's depth (§7.13.3), not a hardcoded 8-bit range.
            *p = i32::from(clip_pixel(
                *p + round2_signed(alpha * (lv - luma_avg), 6),
                self.bit_depth,
            ));
        }
    }
    ...
    fn select_cfl(&self, sx: usize, sy: usize, w: usize, h: usize) -> Option<(i32, i32)> {
        ...
        let best_alpha = |plane: usize| -> i32 {
            ...
            let sad = |alpha: i32| -> i32 {
                ...
                for i in 0..h {
                    for j in 0..w {
                        // The chroma residual block's own extent, and `Clip1` at the frame's depth:
                        // the search must mirror `apply_cfl` on both axes or it scores an alpha the
                        // reconstruction will not produce.
                        let pred = i32::from(clip_pixel(
                            dc + round2_signed(alpha * (l[i * w + j] - luma_avg), 6),
                            self.bit_depth,
                        ));
