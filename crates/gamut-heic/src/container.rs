//! The total, byte-accounting representation of a HEIF file ([`HeifContainer`]).
//!
//! Issue #238's core requirement is that it be *structurally impossible to ignore any bits* in the
//! container: every byte of the input is mapped to exactly one [`Segment`], the segments are
//! contiguous and non-overlapping, and together they cover `0..data.len()` exactly. Unknown
//! top-level boxes are surfaced verbatim (never dropped), an appended foreign stream (a second
//! top-level `ftyp`, e.g. a Samsung motion-photo MP4) is retained as an opaque byte range, and any
//! trailing non-box bytes become an explicit [`SegmentKind::Trailer`].
//!
...
use crate::image::HeifImage;

/// A HEIF file decomposed into a byte-exact list of [`Segment`]s plus the semantic [`HeifImage`].
///
/// # The every-byte invariant
///
/// [`segments`](Self::segments) is contiguous, non-overlapping, and covers `0..data.len()`
/// exactly: `segments[0].range.start == 0`, each `segments[i].range.end == segments[i+1].range.start`,
/// and the last `range.end == data.len()`. This is the "structurally impossible to ignore any bits"
/// guarantee of issue #238 — it holds by construction and is pinned by the crate's tests. Every
/// primary-stream top-level box (including unrecognised ones such as a Google `mpvd`) is a
/// [`SegmentKind::Box`]; an appended foreign stream is a single [`SegmentKind::AppendedStream`]; any
/// trailing non-box bytes are a single [`SegmentKind::Trailer`].
///
...
///
/// Non-exhaustive: a future revision may add a variant (e.g. a typed motion-photo marker) without a
/// breaking change — match with a wildcard arm.
#[derive(Debug, Clone, PartialEq, Eq)]
#[non_exhaustive]
pub enum SegmentKind<'a> {
    /// A top-level box of the primary stream: its four-character type and its body (the bytes after
    /// the 8-byte header). Recognised (`ftyp`/`meta`/`mdat`/`free`/`idat`…) and unrecognised (e.g.
    /// a Google Motion Photo `mpvd`) boxes alike are surfaced here — the container never drops one.
    Box {
    ...
    /// An appended foreign stream: everything from a *second* top-level `ftyp` to end of file, kept
    /// opaque. Real-world phones append a whole second file here (a Samsung motion-photo MP4 with its
    /// own `moov`, optionally followed by a proprietary SEF trailer); its vendor semantics stay
    /// downstream.
    AppendedStream(&'a [u8]),
...
/// A box found inside `meta` (or `meta`'s `iprp`) that the semantic [`gamut_isobmff::read`] does not
/// consume, surfaced verbatim so meta-level accounting is complete.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl<'a> HeifContainer<'a> {
    /// Parses `data` into the total byte-accounting representation and the role-typed semantic view.
    ///
    /// The semantic layer is [`gamut_isobmff::read`] wrapped in a [`HeifImage`], which additionally
    /// validates the primary item at parse time (the `pitm` id must name an existing, non-hidden
    /// item — ISO/IEC 23008-12). The byte-accounting layer walks the top-level boxes into
    /// [`segments`](Self::segments) covering every byte, and shadow-walks `meta` for boxes `read`
    /// does not consume ([`unknown_meta_boxes`](Self::unknown_meta_boxes)).
    ///
    /// # Errors
    ///
    /// Propagates every [`gamut_isobmff::read`] error (a missing/truncated required box, an
    /// out-of-scope feature such as an image sequence, …) and returns
    /// [`Error::InvalidInput`](gamut_core::Error::InvalidInput) if the
    /// primary item is missing or hidden. A malformed top-level box *before* both `ftyp` and `meta`
    /// are seen is a parse error; after that it is retained as a [`SegmentKind::Trailer`].
    pub fn parse(data: &'a [u8]) -> Result<Self> {
        // Semantic parse first: it enforces the stricter rules (rejecting out-of-scope `moov`/`trak`,
        // missing required boxes, …). Because it succeeds only once `ftyp`+`meta` are seen and uses
        // the identical `BoxReader` walk, the byte-accounting walk below cannot then disagree.
        let image = HeifImage::new(read(data)?)?;
...
fn walk_segments(data: &[u8]) -> Result<(Vec<Segment<'_>>, Option<&[u8]>)> {
    ...
    loop {
        ...
        match reader.next_box() {
            Ok(Some(b)) => {
                // A second top-level `ftyp` begins the appended foreign stream: everything from this
                // header to EOF is one opaque segment. The first `ftyp` wins.
            ...
            // Clean end of the box list: the last box tiled exactly to EOF, so the segments already
            // cover every byte.
            Ok(None) => break,
            Err(e) => {
