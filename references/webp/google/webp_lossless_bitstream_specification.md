...
particular block of the ARGB image. The *prediction mode* determines the type of
...
the same techniques described in [Chapter 5](https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#5_image_data).
...
- `width_bits` = 1: For every x value, where x ≡ 0 (mod 2), a green value at x is positioned into the 4 least significant bits of the green value at x / 2, and a green value at x + 1 is positioned into the 4 most significant bits of the green value at x / 2.
- `width_bits` = 2: For every x value, where x ≡ 0 (mod 4), a green value at x is positioned into the 2 least-significant bits of the green value at x / 4, and green values at x + 1 to x + 3 are positioned in order to the more significant bits of the green value at x / 4.
- `width_bits` = 3: For every x value, where x ≡ 0 (mod 8), a green value at x is positioned into the least significant bit of the green value at x / 8, and green values at x + 1 to x + 7 are positioned in order to the more significant bits of the green value at x / 8.
...
`DIV_ROUND_UP`, as defined [earlier](https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#41_predictor_transform).
...
2. Entropy image: Stores the meta prefix codes (see ["Decoding of Meta Prefix Codes"](https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#622_decoding_of_meta_prefix_codes)).
3. Predictor image: Stores the metadata for the predictor transform (see ["Predictor Transform"](https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#41_predictor_transform)).
4. Color transform image: Created by `ColorTransformElement` values (defined in ["Color Transform"](https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#42_color_transform)) for different blocks of the image.
5. Color indexing image: An array of the size of `color_table_size` (up to 256 ARGB values) that stores the metadata for the color indexing transform (see ["Color Indexing Transform"](https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#44_color_indexing_transform)).
...
that order). See [Section 6.2.3](https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#623_decoding_entropy-coded_image_data) for
details.
...
- Distance code is a number indicating the position of a previously seen pixel, from which the pixels are to be copied. The exact mapping is described [below](https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#distance_mapping).
...
[5.2.1](https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#521_prefix_coded_literals) and [5.2.2](https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#522_lz77_backward_reference)).
...
Most of the data is coded using a [canonical prefix code](https://en.wikipedia.org/wiki/Canonical_Huffman_code).
Hence, the codes are transmitted by sending the *prefix code lengths* , as
opposed to the actual *prefix codes*.
...
node using the *normal code length code*, all but one code length are zeros,
...
**Note:** Another special case is when *all* prefix code lengths are *zeros* (an
...
different blocks of the image. *Meta prefix codes* are indexes identifying which
prefix codes to use in different parts of the image.

Meta prefix codes may be used *only* when the image is being used in the
[role](https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#51_roles_of_image_data) of an *ARGB image*.
...
where `DIV_ROUND_UP` is as defined [earlier](https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#41_predictor_transform).
...
the *largest meta prefix code* from the entropy image:
...
Data"](https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#623_decoding_entropy-coded_image_data).
...
`(256 + 24 +` [`color_cache_size`](https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#523_color_cache_coding)`- 1)`.
...
[RFC 5234](https://www.rfc-editor.org/rfc/rfc5234) [RFC 7405](https://www.rfc-editor.org/rfc/rfc7405). It does not cover all details. The end-of-image (EOI)
