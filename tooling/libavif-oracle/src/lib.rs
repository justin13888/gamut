...
/// A decoded planar image: one tightly packed plane per coded component, each sample widened to
/// `u16` (8-bit samples occupy `0..=255`, 10-/12-bit use the wider range).
///
/// gamut emits 4:4:4, 4:2:2, 4:2:0 and monochrome stills. The colour formats carry three planes as
/// `[Y, U, V]` (which under the identity matrix are `G, B, R`), each at its own extent; a
/// monochrome image carries only `Y`, and its two chroma planes are **empty**.
pub struct DecodedImage {
    ...
    /// luma for a subsampled image and empty for a monochrome one. Their dimensions come from
    /// libavif's own accessors, not from a re-derivation here — the point of a differential oracle
    /// is that the geometry is the reference implementation's notion of it.
...
impl DecodedImage {
    /// Whether libavif reported the image as monochrome (`AVIF_PIXEL_FORMAT_YUV400`), in which
    /// case only [`planes`](Self::planes)`[0]` is populated.
    ///
    /// Derived from [`yuv_format`](Self::yuv_format) rather than stored beside it: two fields could
    /// disagree, and the format is the value libavif actually reported.
    #[must_use]
...
/// decoded, or if its pixel format is not one of 4:4:4 / 4:2:2 / 4:2:0 / monochrome, or its depth
/// is not 8, 10 or 12 bits (the forms gamut emits).
...
unsafe fn extract(image: &sys::avifImage) -> Result<DecodedImage, String> {
    ...
    // so the plane *count*, not just each plane's extent, follows the format.
    let coded = match image.yuvFormat {
        sys::AVIF_PIXEL_FORMAT_YUV444
        | sys::AVIF_PIXEL_FORMAT_YUV422
        | sys::AVIF_PIXEL_FORMAT_YUV420 => 3,
        sys::AVIF_PIXEL_FORMAT_YUV400 => 1,
        other => return Err(format!("unexpected pixel format {other}")),
    };
